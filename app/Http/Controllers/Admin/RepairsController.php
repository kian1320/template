...
class RepairsController extends Controller
{
    public function index($repairs_id)
    {
        ...
        return view('admin.repairs.index', ['repairs_id' => $repairs_id, 'Item' => $item]);
    }
...

    {
        ...
        return redirect('admin/repairs/' . $repairs->item_id)->with('message', 'Repair Added');
    }

    public function edit($repairs_id)
    {
        $Repairs = Repairs::find($repairs_id);
        return view('admin.repairs.edit', compact('Repairs'));
    }
    ...
    public function update(RepairsFormRequest $request, $repairs_id)
    {
        ...
        return redirect('admin/repairs/' . $repairs->item_id)->with('message', 'Repair Updated');
    }
    ...
    public function destroy($repairs_id)
    {
        ...
        if ($repairs) {
            $repairs->delete();
            return redirect('admin/repairs/' . $repairs->item_id)->with('message', 'Repair Deleted');
        } else {
            return redirect('admin/repairs/' . $repairs->item_id)->with('message', 'Repair not Deleted');
        }
    }
}
